...
| <a name="input_tags_dest_additional_resources"></a> [tags\_dest\_additional\_resources](#input\_tags\_dest\_additional\_resources) | List of additional resources that need to be tagged when launched via ASG.<br>  Defaults to [] because "instance", "volume" and "network-interface" will always be tagged. | `list(string)` | `[]` | no |

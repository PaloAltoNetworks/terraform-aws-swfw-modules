...
**NOTE 2:**
The Security Group attached to the Management interface uses an inbound rule allowing traffic to port `22` and `443` from dummy `1.1.1.1/32`, which means that SSH and HTTP access to the NFGW is blocked from the Internet. You should update the Security Group rules and open access to the public IP address from which you will connect to VM-Series.
...
6. Upload the configuration files if required e.g. `authcodes` file into `license` folder. More details can be found in [bootstrap package structure guide](https://docs.paloaltonetworks.com/vm-series/11-1/vm-series-deployment/bootstrap-the-vm-series-firewall/bootstrap-package)
7. Initialize Terraform: `terraform init`
8. Prepare plan: `terraform plan`
9. Deploy infrastructure: `terraform apply -auto-approve`
10. Destroy infrastructure if needed: `terraform destroy -auto-approve`

Note:
To deploy the Prisma AIRS product, uncomment the designated (airs_deployment = true) line in the terraform.tfvars file.
To deploy the ARM-based VM-Series, set `arm_deployment = true` and use an ARM-compatible instance type in the terraform.tfvars file.

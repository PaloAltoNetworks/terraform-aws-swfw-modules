...
|------|-------------|------|---------|:--------:|
| <a name="input_alb_target_groups"></a> [alb\_target\_groups](#input\_alb\_target\_groups) | Instances to be the target group for ALB<br><br>  Example:<pre>alb_target_groups = {<br>    app1 = {<br>      vms = ["app1_vm01", "app1_vm02"]<br>    }<br>    app2 = {<br>      vms = ["app2_vm01", "app2_vm02"]<br>    }<br>  }</pre> | `map(any)` | n/a | yes |
...
| <a name="input_spoke_albs"></a> [spoke\_albs](#input\_spoke\_albs) | A map defining Application Load Balancers deployed in spoke VPCs.<br><br>Following properties are available:<br>- `rules`: Rules defining the method of traffic balancing<br>- `vpc`: The VPC in which the load balancer is to be run<br>- `vpc_subnet`: The subnets in which the Load Balancer is to be run<br>- `security_gropus`: Security Groups to be associated with the ALB<pre></pre> | <pre>map(object({<br>    rules           = any<br>    vpc             = string<br>    vpc_subnet      = string<br>    security_groups = string<br>  }))</pre> | n/a | yes |

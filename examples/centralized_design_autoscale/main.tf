...
module "iam" {
  ...
  create_lambda_policy     = try(each.value.create_lambda_policy, false)
  delicense_ssm_param_name = try(each.value.delicense_ssm_param_name, null)
  principal_role           = try(each.value.principal_role, "ec2.amazonaws.com")
}
